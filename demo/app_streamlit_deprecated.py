...
def recursive_selectbox(valid, keys, select={}, prefix="", cols=None, col_count=0, container=None):
    ...
    if len(init) > 1:
        with cols[col_count % len(cols)]:
            with container:
                select[k] = st.selectbox(f"{k}", options=np.unique(init + ["Unknown or Empty"]), index=None, key=f"{prefix}_dropdown_{k}")
    else:
        select[k] = init[0]
    if select[k]:
        col_count += 1
            ...
            if key not in combinations_keys:
                ...
                with cols[col_count % col]:
                    if characteristic == "any":
                        ...
                        char = [elt for elt in characteristic if elt and elt != "Unknown or Empty"] + ["Unknown or Empty"]
...
if __name__ == "__main__":
    ...
    tab_names = ["Général", "Parois", "Vitrages", "Ponts thermiques", "ECS", "Climatisations", "Chauffages", "Résultat"]
